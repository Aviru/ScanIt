...
NSString *const kAviSpinnerTypeActivityIndicator = @"AviActivityIndicator";
NSString *const kAviSpinnerTypeCircular = @"AviCircularSpinner";
NSString *const kAviSpinnerTypeBeachBall = @"AviBeachBallSpinner";
...
#pragma mark -
#pragma mark -
#pragma mark AviActivityIndicator 

@interface AviActivityIndicator : AviSpinner
{
...
@implementation AviActivityIndicator
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        ...
        _strokeIndex = 0;
        if (self.spinType == AviSpinTypeDraw) {_animationTimer = nil;}
        self.numberOfStrokes = 12;
        self.strokeWidth = 2.00;
        self.patternStyle = AviActivityIndicatorPatternStyleSolid;
...
- (void)dealloc
{
    if (self.spinType == AviSpinTypeDraw) {[_animationTimer release];}
    [super dealloc];
}
...
- (void)startAnimating
{
    ...
    _strokeIndex = 1;
    if (self.spinType == AviSpinTypeDraw) {
...
- (void)drawRect:(CGRect)rect
{
    ...
    for (int i=0; i<self.numberOfStrokes; i++) 
    {
        ...
        if (nil == self.segmentImage) 
        {
            ...
            {
                case AviActivityIndicatorPatternStyleSolid:
                    CGContextSetLineDash(context,0,0,0);
                    [self drawStrokeToPoint:CGPointMake(x2, y2) context:context];
                    break;
                    
                case AviActivityIndicatorPatternStyleDash:
                ...
                case AviActivityIndicatorPatternStyleDot:
                ...
                case AviActivityIndicatorPatternStyleDashDot:
                    dash[0] =  self.strokeWidth*3;
                    dash [1] = self.strokeWidth;
...
#pragma mark -
#pragma mark -
#pragma mark AviCircularSpinner
@interface AviCircularSpinner : AviSpinner
{
    CGFloat _angle;
...
@implementation AviCircularSpinner
#pragma mark -
...
#pragma mark -
#pragma mark -
#pragma mark AviBeachBallSpinner
@interface AviBeachBallSpinner : AviSpinner
{
...
@implementation AviBeachBallSpinner
...
#pragma mark -
#pragma mark -
#pragma mark AviSpinner

@implementation AviSpinner
...
//Synthesizer specific to kAviSpinnerTypeActivityIndicator
...
//Synthesizer specific to kAviSpinnerTypeCircular
@synthesize fillColor = _fillColor;
@synthesize pathColor = _pathColor;
@synthesize thickness = _thickness;


//Synthesizer specific to kAviSpinnerTypeBeachBall
...
- (AviSpinner *)initWithSpinnerType:(NSString *) spinnerType
{
    ...
    if (self) 
    {
        ...
        if([self isKindOfClass:[AviActivityIndicator class]])
        {
            self.spinType = AviSpinTypeDraw;
        }
    }
    return self;
}
...
- (void)startRotating
{
    ...
    if ([self isKindOfClass:[AviCircularSpinner class]])
    {
//        dur = 1.0/2.0;
    }
    else if ([self isKindOfClass:[AviActivityIndicator class]])
    {
//        dur = 1.0;
    }
    ...
    [self.layer addAnimation:rotate
                      forKey:@"AviSpinnerRotationAnimation"];
}

- (void)stopRotating
{
    [self.layer removeAnimationForKey:@"AviSpinnerRotationAnimation"];
}
...
//    NSLog(@"Start Animation");
    ...
    if (self.spinType == AviSpinTypeRotate)
    {
        [self startRotating];
    }
}
